...
const Project = () => {
  ...
  return (
    ...
    >
        ...
        {hasil
          ? project.map((item, i) => {
              if (i < 8) {
                return (
                  <div
                    className="h-auto bg-white shadow-xl rounded-xl border border-blue"
                    data-aos={item.aos}
                  ...
                  >
                    ...
                    <div className="mt-1 p-3 flex flex-col justify-between gap-5">
                      <h1 className=" font-bold text-[18px] lg:text-[20px] text-black">
                        {item.name}
                      </h1>
                      <a
                        href={item.link}
                        target="_blank"
                        className="text-[16px] md:text-sm lg:text-[16px] text-blue underline"
                      >
                        See Preview
                      </a>
                    </div>
      ...
      <Button1 name={hasil ? 'See More ' : 'See Less'} onclick={seeMore} className='mt-10'/>
    </div>
  );
};
